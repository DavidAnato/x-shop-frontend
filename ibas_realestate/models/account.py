...
class IBASAccount(models.Model):
    ...
    is_user_collection = fields.Boolean(string="User a Collector?", compute='_compute_user_rights')
    ...
    def _compute_user_rights(self):
        for rec in self:
            rec.is_user_collection = False
            if self.env.ref('ibas_realestate.group_account_collection').id in self.env.user.groups_id.ids:
                rec.is_user_collection = True            
    ...
    discount_rate = fields.Float('Discount Amount', digits=(16, 2), readonly=True, compute='_compute_discount_rate')
